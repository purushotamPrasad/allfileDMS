...
function GroupPermission({ groupData ,open, setAlertHandler }: listProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await get<any>(`/permission/getAllPermissionByGroupId/${gid}`, {}, 'instance1', setAlertHandler);
    ...
    const deleteApi = async (id: number) => {
        try {
            const response = await del(`/permission/deletePermissionByGroupIdAndPermissionId/${gid}`, {}, {
