...
interface PermissionTabData {
  id: string,
  IndentityPermissionId: string,
  name: string,
  description: string,
  status: string
}
...
function Permission({ filterUI }: listProps) {
  const routers = useRouter();
  const theme = useTheme();
  const [selectedRowArray, setSelectedRowArray] = React.useState<GridValidRowModel[]>([]);
  const [PermissionTabData, setPermissionTabData] = useState<PermissionTabData[]>([]);
  ...
  const rowData: GridRowsProp = [
    {
      ...
      description: "New Permissions Logged Successfully"
    },
    {
      id: 2,
      PermissionId: 123456,
      ...
      role: "Sub Admin",
      timeStamp: "14 Apr 2024 6:42 ",
      description: "Permissions Revoked, Audit Recorded"
    },]
  const columnData: GridColDef[] = [
    {
      ...
      renderCell: (params) => (
        ...
        // onClick={() => { routers.push(`/Permission-configuration/SSO-Configurations/provider-details/?PermissionId=${params.row.PermissionId}`) }}
        >
          {params.value}
        </Box>
      ),
