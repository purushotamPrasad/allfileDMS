...
interface SystemTabData {
  id: string,
  IndentitySystemId: string,
  name: string,
  description: string,
  status: string
}
...
function System({ filterUI }: listProps) {
  const routers = useRouter();
  const theme = useTheme();
  const [selectedRowArray, setSelectedRowArray] = React.useState<GridValidRowModel[]>([]);
  const [SystemTabData, setSystemTabData] = useState<SystemTabData[]>([]);
  ...
  const rowData: GridRowsProp = [
    {
      ...
      description: "Timing Parameters Set"
    },
    {
      id: 2,
      SystemId: 123456,
      ...
      changedValue: "4",
      timeStamp: "14 Apr 2024 6:42 ",
      description: "Information, Data, Material,Substance"
    },]
  const columnData: GridColDef[] = [
    {
      ...
      renderCell: (params) => (
        ...
        // onClick={() => { routers.push(`/system-configuration/SSO-Configurations/provider-details/?SystemId=${params.row.SystemId}`) }}
        >
          {params.value}
        </Box>
      ),
