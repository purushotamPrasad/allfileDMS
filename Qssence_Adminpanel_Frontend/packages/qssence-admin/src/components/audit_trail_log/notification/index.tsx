...
interface NotificationTabData {
  id: string,
  IndentityNotificationId: string,
  name: string,
  description: string,
  status: string
}
...
function Notification({ filterUI }: listProps) {
  const routers = useRouter();
  const theme = useTheme();
  const [selectedRowArray, setSelectedRowArray] = React.useState<GridValidRowModel[]>([]);
  const [NotificationTabData, setNotificationTabData] = useState<NotificationTabData[]>([]);
  ...
  const rowData: GridRowsProp = [
    {
      ...
      description: "Security Alert Recorded Successfully"
    },
    {
      id: 2,
      NotificationId: 123456,
      name: "Ayush@gmail.com",
      event: "Compliance Alerts",
      role: "Sub Admin",
      timeStamp: "14 Apr 2024 6:42 ",
      description: "Compliance Alert Logged Successfully"
    },]
  const columnData: GridColDef[] = [
    {
      ...
      renderCell: (params) => (
        ...
        // onClick={() => { routers.push(`/Notification-configuration/SSO-Configurations/provider-details/?NotificationId=${params.row.NotificationId}`) }}
        >
          {params.value}
        </Box>
      ),
